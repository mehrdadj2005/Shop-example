...
interface EditAddressProps {
  editAddressModal: string;
  setEditAddressModal: (value: { show: boolean; text: string }) => void;
}
...
function NewAddress({ newAddressModal, setNewAddressModal }: NewAddressProps) {
  ...
  const AddAddressToLS = () => {
    const addresses = localStorage.getItem("addresses");
    console.log(address);
    const allAddresses: string[] = addresses?.length
      ? JSON.parse(addresses)
      : [];
    const findingAddress = allAddresses?.find((item) => item == address);
    !findingAddress?.length
      ? localStorage.setItem(
          "addresses",
          JSON.stringify([...allAddresses, address])
        )
      : "";
    ...
    <div className="w-full h-screen bg-[#00000040] absolute top-0 ring-0 left-0">
      <div className="bg-white rounded-2xl w-10/12 h-80 absolute top-1/6 left-0 right-0 mx-auto flex items-center px-2 flex-col py-4">
        ...
        <div className="w-full px-4">
          <Textarea
            className="min-h-36 max-h-36 border-2 border-primary text-stone-700"
            value={address}
...
function EditAddress({
  ...
  useEffect(() => {
    setAddress(editAddressModal);
  }, [editAddressModal]);

  const cheangeAddressToLS = () => {
    const addresses = localStorage.getItem("addresses");
    const allAddresses: string[] = addresses?.length
      ? JSON.parse(addresses)
      : [];
    allAddresses.map((item, index) => {
      if (item == editAddressModal) {
        allAddresses.splice(index, 1, address);
        localStorage.setItem("addresses", JSON.stringify(allAddresses));
      }
    });
    setEditAddressModal({ show: false, text: "" });
  };
    ...
    <div className="w-full h-screen bg-[#00000040] absolute top-0 ring-0 left-0">
      <div className="bg-white rounded-2xl w-10/12 h-80 absolute top-1/6 left-0 right-0 mx-auto flex items-center px-2 flex-col py-4">
        <div className="w-full flex justify-between">
          ...
          <Image
            onClick={() => setEditAddressModal({ show: false, text: "" })}
            className="mr-2 ml-auto mb-6"
            src={icons.close}
            alt="arrow left"
            width={24}
